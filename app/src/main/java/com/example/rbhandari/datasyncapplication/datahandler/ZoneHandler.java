...
public class ZoneHandler {
    ...
    public ZoneHandler(String parseId, String name, String type, Long auditId, Date created, Date updated){
        try {
    ...
    public static void updateLocalZone(String objectId, String username) {
        try{
            User user = (User) (ZoneHandler.getZoneById(username)).get(0);
    ...
    public static JSONArray getUserByParseId(String parseid){
        List<Zone> zones = Zone.find(Zone.class,"parseid=?",parseid);
    ...
    private static JSONArray getAllParseIdNotSetZones(){
        List<Zone> zones = Zone.find(Zone.class,"parseid=?","");
        JSONArray zoneArray = new JSONArray();
        for(int i = 0;i<zones.size();i++)
        {
            Zone user = zones.get(i);
            try
            {
                zoneArray.put(user);
            }
